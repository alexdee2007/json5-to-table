...
test("fill null", (t) => {
  ...
  t.deepEqual(matrix.toArray(), [[{ val: undefined, rowSpan: 1, colSpan: 1 }]]);
});
...
test("fill full value", (t) => {
  ...
  fillData(data, schema, matrix);
  t.deepEqual(matrix.toArray(), [[{ val: 1, rowSpan: 1, colSpan: 1 }]]);
});
...
test("fill into crossing rows", (t) => {
  ...
  fillData(data, schema, matrix);
  t.deepEqual(matrix.toArray(), [[{ val: 1, rowSpan: 2, colSpan: 1 }], [undefined]]);
});
...
test("fill object array", (t) => {
  ...
  fillData(data, schema, matrix);
  t.deepEqual(matrix.toArray(), [[{ val: 1, rowSpan: 1, colSpan: 1 }], [{ val: 2, rowSpan: 1, colSpan: 1 }]]);
});
...
test("fill nested properties", (t) => {
  const schema = { path: "a", props: [{ path: "b" }, { path: "c" }] };
...
test("fill array with nested properties", (t) => {
  ...
  t.deepEqual(matrix.toArray(), [
    ...
    [undefined, undefined],
    [
      { val: 3, rowSpan: 2, colSpan: 1 },
      { val: 4, rowSpan: 2, colSpan: 1 }
    ],
    [undefined, undefined]
  ]);
});
...
test("fill when schema is array and the first data property is array", (t) => {
  const schema = [{ path: "a", props: [{ path: "a1" }, { path: "a2" }] }, { path: "b" }];
  const data = {
    a: [
      { a1: 1, a2: 2 },
      { a1: 3, a2: 4 }
  ...
  t.deepEqual(matrix.toArray(), [
    ...
    [{ val: 3, rowSpan: 1, colSpan: 1 }, { val: 4, rowSpan: 1, colSpan: 1 }, undefined]
  ]);
});

test("fill when schema is array and the first schema item occupy multiple colomns", (t) => {
  const schema = [{ path: "a", props: [{ path: "b" }, { path: "c" }] }, { path: "d" }];
...
test("fill empty array into multiple rows", (t) => {
  ...
  fillData(data, schema, matrix);
  t.deepEqual(matrix.toArray(), [[{ val: undefined, rowSpan: 2, colSpan: 1 }], [undefined]]);
});
...
test("one property is an empty array", (t) => {
  const schema = [
    { path: "a" },
    { path: "b", props: [{ path: "b1" }, { path: "b2" }] },
    { path: "c", props: [{ path: "c1" }, { path: "c2" }] }
  ];
  const data = [
    {
      a: 1,
  ...
  t.deepEqual(matrix.toArray(), [
    ...
    [undefined, undefined, undefined, { val: 4, rowSpan: 1, colSpan: 1 }, { val: 5, rowSpan: 1, colSpan: 1 }]
  ]);
});
...
test("every data item occupies different rows", (t) => {
  ...
  const schema = [{ path: "items", props: [{ path: "text" }] }];
